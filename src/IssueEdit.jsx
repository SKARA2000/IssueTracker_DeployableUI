...
class IssueEdit extends React.Component{
    ...
    async loadData(){
        ...
        this.setState({
            issue: data ? data.issue: {}, invalidFields: {}
        });
    }
    ...
    render(){
        ...
        if(id == null){
            if(propId != null){
                return(<h3>{ `Issue with property ${propId} not found` }</h3>)
            }
            return null;
        }
        ...
        if(Object.keys(invalidFields).length !== 0 && showingValidation){
            validateMessage = (
                <Alert bsStyle="danger" onDismiss={this.dismissValidation}>
                    Please correct incorrect fields before submitting
                </Alert>
            );
        }

        const { issue: { title, status, owner } } = this.state;
